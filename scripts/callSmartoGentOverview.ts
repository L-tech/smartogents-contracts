...
import ABI from "../abis/SmartoGentOverview.json";
...
async function main() {
  ...
  const contractAddress = process.env.SMARTO_GENT_OVERVIEW
  const registryContract = process.env.CONTRACT_ABI_REGISTRY
  if (!contractAddress) throw Error("Missing SMARTO_GENT_OVERVIEW in .env")
  ...
  const transactionResponse = await contract.initiateRequest(message, registryContract)
  const receipt = await transactionResponse.wait()
  console.log(`Message sent, tx hash: ${receipt.hash}`)
  console.log(`Chat started with message: "${message}"`)

  // Get the chat ID from transaction receipt logs
  let chatId = getRequestId(receipt, contract);
  console.log(`Created Request ID: ${chatId}`)
  if (!chatId && chatId !== 0) {
    return
  }
    ...
    if (newMessages) {
      for (let message of newMessages) {
        ...
        if (allMessages.at(-1)?.role == "assistant") {
          ...
          const transactionResponse = await contract.askQuestion(message, chatId, registryContract)
          const receipt = await transactionResponse.wait()
          console.log(`Message sent, tx hash: ${receipt.hash}`)
        }
      }
...
async function getUserInput(): Promise<string | undefined> {
  ...
  try {
    const input = await question("Ask SmartoGent: ")
    rl.close()
    return input
  } catch (err) {
...
function getRequestId(receipt: TransactionReceipt, contract: Contract) {
  let chatId
