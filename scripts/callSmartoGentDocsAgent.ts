...
import ABI from "../abis/SmartoGentDocsAgent.json"
...
async function main() {
    ...
    const contractAddress = process.env.SMARTO_GENT_DOCS_AGENT
  ...
  const transactionResponse = await contract.runAgent(query, Number(maxIterations), registryContract);
